...
CHECK_OK = (
    "execute if score #_bl_returning _bl matches 1 if score #_bl_success _bl matches 1 "
    "run return run scoreboard players get #_bl_value _bl"
)
CHECK_FAIL = "execute if score #_bl_returning _bl matches 1 if score #_bl_success _bl matches 0 run return fail"
...
""")
    ...
    assert out.file_contents == {
        "data/pack/function/main/f.mcfunction": "\n".join(
            [RESET, _stores("execute as @p", "pack:main/f_helper/as_0"), CHECK_OK, CHECK_FAIL, "say never"]
        ),
        "data/pack/function/main/f_helper/as_0.mcfunction": "\n".join([SET_RETURNING, "return 42"]),
    }
...
def test_two_level_nesting_matches_the_complete_return_example():
    ...
    # emitted (it's the runtime check that skips it, not the compiler).
    out = compile_source("""\
...
""")
    ...
    assert out.file_contents == {
        "data/pack/function/main/complete_return.mcfunction": "\n".join(
            [
                RESET,
                _stores("execute as @p", "pack:main/complete_return_helper/as_0"),
                CHECK_OK,
                CHECK_FAIL,
                "say This will never run.",
            ]
        ...
        "data/pack/function/main/complete_return_helper/as_0.mcfunction": "\n".join(
            [
                _stores("execute at @s", "pack:main/complete_return_helper/as_0_helper/at_0"),
                CHECK_OK,
                CHECK_FAIL,
            ]
        ),
        "data/pack/function/main/complete_return_helper/as_0_helper/at_0.mcfunction": "\n".join(
            [SET_RETURNING, "return 42"]
        ),
    }
...
""")
    ...
    assert CHECK_OK in as_0
    assert CHECK_FAIL in as_0
...
""")
    assert out.errors == []
    assert out.file_contents["data/pack/function/main/f_helper/as_0.mcfunction"] == "\n".join(
        [SET_RETURNING, "return run scoreboard players get #x obj"]
    )
...
""")
    ...
    assert out.file_contents == {
        "data/pack/function/main/f.mcfunction": "\n".join(
            [RESET, _stores("execute if entity @s", "pack:main/f_helper/return_0"), CHECK_OK, CHECK_FAIL, "say after"]
        ),
        "data/pack/function/main/f_helper/return_0.mcfunction": "\n".join([SET_RETURNING, "return 7"]),
    }
...
def test_inline_return_prefix_is_evaluated_exactly_once_not_duplicated():
    ...
    # only ever gets evaluated by the single `execute summon ... run function ...` call.
    out = compile_source("""\
function f:
...
""")
    ...
    assert "execute summon minecraft:marker store result" in top
...
""")
    ...
    assert out.file_contents == {
        "data/pack/function/main/f.mcfunction": "\n".join(
            [RESET, _stores("execute as @p", "pack:main/f_helper/as_0"), CHECK_OK, CHECK_FAIL, "say after"]
        ),
        "data/pack/function/main/f_helper/as_0.mcfunction": "\n".join(
            [
                _stores("execute if entity @s", "pack:main/f_helper/as_0_helper/return_0"),
                CHECK_OK,
                CHECK_FAIL,
            ]
        ),
...
""",
    ...
    assert out.errors == []
    lines = out.file_contents["data/pack/function/main/f.mcfunction"].splitlines()
...
""")
    ...
    assert top.startswith(RESET)
    for level in (top, as_0, at_0):
        assert CHECK_OK in level
        assert CHECK_FAIL in level
    assert positioned_0 == "\n".join([SET_RETURNING, "return 5"])
